  ...
  try {
    ...
    if (!options.dryRun) {
      ...
      if (results.successful.length > 0) {
        ...
        console.log('💡 Run "stacksorbit monitor" to track deployment confirmation');
      }
    }
...
async function runChecks(args) {
  ...
  try {
    ...
    console.log(`${allPassed ? '✅' : '❌'} Overall status: ${allPassed ? 'READY' : 'ISSUES FOUND'}`);
...
async function verifyDeployment(args) {
  ...
  try {
    ...
    // Verify contracts
    const deployedContracts = await monitor.getDeployedContracts(config.SYSTEM_ADDRESS);
    console.log('');

    // Check deployment manifest
    const manifestPath = path.join(process.cwd(), 'deployment', 'testnet-manifest.json');
