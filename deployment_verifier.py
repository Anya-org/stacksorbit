...
class DeploymentVerifier:
    ...
    def run_comprehensive_verification(self, expected_contracts: Optional[List[str]] = None) -> Dict:
        ...
        # Overall status
        self.verification_results['overall_status'] = 'success' if all_passed else 'failed'
    ...
    def _verify_api_connectivity(self, address: str, expected_contracts: Optional[List[str]] = None) -> Dict:
    ...
    def _verify_account_status(self, address: str, expected_contracts: Optional[List[str]] = None) -> Dict:
    ...
    def _verify_contract_deployment(self, address: str, expected_contracts: Optional[List[str]] = None) -> Dict:
    ...
    def _verify_transaction_history(self, address: str, expected_contracts: Optional[List[str]] = None) -> Dict:
        ...
        try:
            ...
            recent_txs = [tx for tx in transactions if
                         datetime.fromisoformat(tx['burn_block_time'].replace('Z', '+00:00')) >
                         datetime.now() - timedelta(hours=24)]

            # Look for contract deployment transactions
            deploy_txs = [tx for tx in recent_txs if tx.get('tx_type') == 'contract_call']
    ...
    def _verify_network_health(self, address: str, expected_contracts: Optional[List[str]] = None) -> Dict:
        """Verify network health and performance"""
        api_status = self.monitor.check_api_status()

        # Check if network is healthy
        passed = (api_status['status'] == 'online' and
                 api_status.get('block_height', 0) > 0)
    ...
    def _verify_gas_usage(self, address: str, expected_contracts: Optional[List[str]] = None) -> Dict:
    ...
    def _verify_contract_functionality(self, address: str, expected_contracts: Optional[List[str]] = None) -> Dict:
        ...
        if not deployed_contracts:
            return {
                'passed': False,
                'error': 'No contracts deployed to test'
            }
    ...
    def _generate_recommendations(self):
        ...
        if not contract_check.get('passed'):
            ...
            if missing:
                recommendations.append(f"Deploy missing contracts: {', '.join(missing)}")
