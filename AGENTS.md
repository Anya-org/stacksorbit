...
* **CLI (`conxius_orbit_cli.py`):** The primary entry point for all commands. It parses user input and delegates tasks to the appropriate modules.
...
* **Python:** This project follows the [PEP 8](https://pep8.org/) style guide and uses [Black](https://black.readthedocs.io/) for formatting.
    ...
    ```bash
    # Format code
    black conxius_orbit.py

    # Lint
    pylint conxius_orbit.py

    # Type check
    mypy conxius_orbit.py
    ```
...
2. **Run Pre-Deployment Checks:**
    ...
    ```bash
    python conxius_orbit_cli.py diagnose --verbose
    ```
...
4. **Deploy to Testnet:**
    ...
    ```bash
    python conxius_orbit_cli.py deploy --network testnet
    ```
...
5. **Monitor the Deployment:**
    ...
    ```bash
    python conxius_orbit_cli.py monitor --follow
    ```
...
* **Deploy a Specific Category:**
    ...
    ```bash
    python conxius_orbit_cli.py deploy --category core
    ```
...
* **Auto-Detect Contracts:**
    ...
    ```bash
    python conxius_orbit_cli.py detect --directory .
    ```

* **Apply a Deployment Template:**

    ```bash
    python conxius_orbit_cli.py template --name <template_name>
    ```

* **Manage Local Devnet:**

    ```bash
    python conxius_orbit_cli.py devnet --devnet-command <start|stop|status>
    ```
...
* **`Missing required configuration`:** This error occurs when one of the required variables (`DEPLOYER_PRIVKEY`, `SYSTEM_ADDRESS`, `NETWORK`) is not set in your `.env` file. Run `python conxius_orbit_cli.py setup` to regenerate the file.
...
* **`Pre-deployment checks failed`:** Run `python conxius_orbit_cli.py diagnose --verbose` to get a detailed report of the issues.
