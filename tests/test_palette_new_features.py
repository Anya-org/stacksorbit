...
async def test_relative_time_formatting():
    ...
    # Test '5m ago'
    five_m_ago = (
        (now_utc - timedelta(minutes=5, seconds=5)).isoformat().replace("+00:00", "Z")
    )
    ...
    # Test '2h ago'
    two_h_ago = (
        (now_utc - timedelta(hours=2, seconds=5)).isoformat().replace("+00:00", "Z")
    )
    ...
    # Test '1d ago'
    one_d_ago = (
        (now_utc - timedelta(days=1, hours=1)).isoformat().replace("+00:00", "Z")
    )
...
async def test_balance_colorization():
    ...
    async with app.run_test() as pilot:
        ...
        app.monitor.get_account_info = MagicMock(
            return_value={"balance": 2000000, "nonce": 5}
        )
...
async def test_transaction_confirmations_and_highlighting():
    ...
    async with app.run_test() as pilot:
        ...
        app.on_transactions_row_highlighted(
            DataTable.RowHighlighted(table, table.get_row_index(tx_id), RowKey(tx_id))
        )
        assert app.selected_tx_id == tx_id
        assert not app.w_copy_tx_btn.disabled
        assert "(6)" in str(app.w_tx_status_label.render()) or "Selected" in str(
            app.w_tx_status_label.render()
        )
    ...
    async with app.run_test() as pilot:
        ...
        app.on_contracts_row_selected(
            DataTable.RowSelected(
                contracts_table,
                contracts_table.get_row_index("empty-deploy"),
                RowKey("empty-deploy"),
            )
        )
        await pilot.pause(0.1)
        ...
        app.on_transactions_row_selected(
            DataTable.RowSelected(
                transactions_table,
                transactions_table.get_row_index("empty-refresh"),
                RowKey("empty-refresh"),
            )
        )
        await pilot.pause(0.1)
        ...
        app.on_contracts_row_selected(
            DataTable.RowSelected(
                contracts_table,
                contracts_table.get_row_index("empty-settings"),
                RowKey("empty-settings"),
            )
        )
        await pilot.pause(0.1)
