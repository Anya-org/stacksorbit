...
class TestVerifierApiConnectivity(unittest.TestCase):
    @patch("scripts.deployment_verifier.DeploymentMonitor.check_api_status")
    @patch("scripts.deployment_verifier.DeploymentMonitor.get_account_info")
    @patch("scripts.deployment_verifier.DeploymentMonitor.get_deployed_contracts")
    def test_api_offline_marks_failure(self, mock_deployed, mock_account, mock_api):
        ...
        verifier = DeploymentVerifier(
            network="testnet",
            config={"SYSTEM_ADDRESS": "ST1PQHQKV0RJXZFY1DGX8MNSNYVE3VGZJSRTPGZGM"},
        )
