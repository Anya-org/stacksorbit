...
def run_benchmark():
    ...
    print(
        f"Total time saved per UI refresh (cached): ~{0.2:.1f}s (2 x 100ms simulated latency)"
    )
    print(
        f"Parallelization benefit in GUI: latency is max(T_blockchain, T_infra) instead of T_blockchain + T_infra_1 + T_infra_2"
    )
