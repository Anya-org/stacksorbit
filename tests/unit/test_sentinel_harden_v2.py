...
class TestSentinelHardenV2(unittest.TestCase):
    ...
    def test_expanded_mnemonic_lengths(self):
        ...
        for length, m in mnemonics.items():
            self.assertTrue(
                is_sensitive_value(m), f"{length}-word mnemonic should be sensitive"
            )

        # Also verify 12 and 24 still work
        self.assertTrue(
            is_sensitive_value(
                "art forum decade glow blue case mouse track merit label logic adjust"
            ),
            "12-word mnemonic should be sensitive",
        )
        self.assertTrue(
            is_sensitive_value(
                "art forum decade glow blue case mouse track merit label logic adjust total script real blue case mouse track merit label logic adjust total"
            ),
            "24-word mnemonic should be sensitive",
        )
    ...
    def test_redaction_with_new_keywords(self):
        ...
        config = {
            ...
            "nested": {"ADMIN_TOKEN": "super-secret-token"},
        }
