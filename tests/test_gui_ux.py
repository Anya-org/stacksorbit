...
async def test_transaction_selection_copies_to_clipboard():
    ...
    with patch.object(app, "notify") as mock_notify, patch.object(
        ...
        async with app.run_test() as pilot:
            ...
            tx_id = (
                "0x_mock_transaction_id_for_testing_purposes_only_1234567890abcdef123"
            )

            # Manually add a row with a key
            transactions_table.add_row(
                tx_id[:10] + "...", "contract_call", "success", "123", key=tx_id
            )
            await pilot.pause()
            ...
            app.on_transactions_row_selected(
                transactions_table.RowSelected(
                    data_table=transactions_table, row_key=RowKey(tx_id), cursor_row=0
                )
            )
            await pilot.pause()
